...
class UnalignedDataset(BaseDataset):
    ...
    def tiff_open(self, imgPath):
        ...
        if num_bands == 1:
            ...
            band1 = np.expand_dims(np.array(self.image.GetRasterBand(1).ReadAsArray()) , axis=2)
            band2 = np.expand_dims(np.array(self.image.GetRasterBand(2).ReadAsArray()) , axis=2)
            band3 = np.expand_dims(np.array(self.image.GetRasterBand(3).ReadAsArray()) , axis=2)
            self.img_array = np.concatenate([band1, band2, band3], axis=2)
        else:
