...
class ResnetBlock_cond(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias,temb_dim,z_dim):
        ...
        self.conv_block += [FourierBlock(1, dim)]
        ...
        self.conv_fin += [FourierBlock(1, dim)]
    ...
    def forward(self, x,time_cond,z):
        ...
        # print('---------------------------')
        for n,layer in enumerate(self.conv_fin):
...
class Spectra(nn.Module):
    def __init__(self,in_depth,AF='prelu'):
        ...
        self.inter_depth = self.in_depth//2 if in_depth>=2 else self.in_depth  # Fourier
...
class FastFC(nn.Module):
    def __init__(self,in_depth,AF='prelu'):
        ...
        self.conv_ll = nn.Conv2d(self.in_depth,self.in_depth,3,padding=1)
        self.conv_lg = nn.Conv2d(self.in_depth,self.in_depth,3,padding=1)
        self.conv_gl = nn.Conv2d(self.in_depth,self.in_depth,3,padding=1)
...
class FourierBlock(nn.Module):
    def __init__(self,num_layer,in_depth,return_all=False, attention=True,):
        super().__init__()
        #Params
